...
export default function DataRow({ index, title, category, year, href, children }: DataRowProps) {
  const row = (
    ...
    >
      ...
      {category && (
        <span className="hidden sm:block w-32 truncate text-text-secondary">{category}</span>
      )}
      {year && (
        <span className="w-12 text-right text-text-secondary">{year}</span>
      )}
      <span
        className="w-4 text-right transition-colors group-hover:text-accent text-border"
      >
        →
      </span>
    </div>
