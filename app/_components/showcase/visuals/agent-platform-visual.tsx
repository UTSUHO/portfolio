...
const flowNodes = [
  { x: 40, label: 'HANDSHAKE', n: '01' },
  { x: 120, label: 'AUTH', n: '02' },
  { x: 200, label: 'STREAM', n: '03' },
  { x: 280, label: 'TOOL_CALL', n: '04' },
  { x: 360, label: 'CLOSE', n: '05' }
]
...
function setNodeVisuals(
  ...
  if (core) {
    core.setAttribute('class', active ? `${styles['accent-fill']} ${styles.pulse}` : styles['fill-base'])
  }
}
...
export default function AgentPlatformVisual() {
  ...
  return (
    ...
    >
      ...
      <rect x={24} y={40} width={110} height={140} className={`${styles['line-base']} ${styles['fill-panel']}`} />
      ...
      {wsRows.map((row, i) => {
        ...
        return (
          <g key={row}>
            {selected && (
              <rect x={27} y={y + 1} width={104} height={18} className={styles['fill-base']} stroke="none" />
            )}
            <rect
              x={33}
              y={y + 7}
              ...
              height={5}
              fill="none"
              className={selected ? styles['line-strong'] : styles['line-faint']}
            />
            <text x={44} y={y + 12} fontSize={6.5} className={selected ? styles['text-strong'] : styles['text-label']}>
              {row}
            </text>
          </g>
        )
      ...
      />
      <path d="M164 106 L170 110 L164 114" fill="none" className={styles['line-base']} />
      ...
      />
      <circle cx={195} cy={110} r={3.5} className={`${styles['fill-base']} ${styles.pulse}`} stroke="none" />
      <text x={195} y={152} fontSize={7} textAnchor="middle" className={styles['text-label']}>
        WS GATEWAY
      </text>
      ...
      />
      <path d="M252 106 L258 110 L252 114" fill="none" className={styles['line-base']} />

      <rect x={258} y={40} width={118} height={140} fill="none" className={styles['line-base']} />
      <line x1={258} y1={58} x2={376} y2={58} className={styles['line-faint']} />
      ...
      {[
        ...
        <g key={slot.name}>
          ...
          />
          {slot.active && (
            <rect x={266} y={slot.y} width={102} height={26} stroke="none" className={styles['accent-soft']} />
          )}
          <text
            x={274}
            y={slot.y + 16}
            fontSize={6.5}
            className={slot.active ? styles['text-accent'] : styles['text-label']}
          >
            {slot.name}
          </text>
          <circle
            ...
            r={2}
            stroke="none"
            className={slot.active ? `${styles['accent-fill']} ${styles.pulse}` : styles['fill-base']}
          />
        </g>
      ))}
      <text
      ...
      <line x1={24} y1={216} x2={376} y2={216} className={styles['line-faint']} />
      <text x={24} y={232} fontSize={7} className={styles['text-label']}>
        MESSAGE FLOW
      </text>

      <line x1={startX} y1={252} x2={endX} y2={252} className={styles['line-faint']} />
      <path
      ...
      {flowNodes.map((node, i) => (
        <g key={node.n}>
          ...
          />
          <text x={node.x} y={244} fontSize={6} textAnchor="middle" className={styles['text-label']}>
            {node.label}
          </text>
          <text x={node.x} y={266} fontSize={6} textAnchor="middle" className={styles['text-label']}>
            {node.n}
          </text>
        </g>
      ))}
